...
# This dictionary will now only hold configurations for the "plant_predict" dataset
DATASET_CONFIGS = {
    "plant_predict": {
        "SAVED_MODEL_DIR": Path("./03_morphometrics_output_plant_predict/"), # Your existing morphometrics output
        "PCA_PARAMS_FILE_NAME": "leaf_pca_model_parameters_plant_predict.h5",
        "PCA_SCORES_LABELS_FILE_NAME": "original_pca_scores_and_class_labels_plant_predict.h5",
        "EXCLUDE_CLASSES": [], # No classes to exclude for plant_predict based on previous conversation
        "SYNTHETIC_DATA_OUTPUT_BASE_DIR": Path("./05_synthetic_leaf_data_plant_predict/"), # New unique output dir
        "DATASET_FULL_NAME": "Plant Predict Collection",
        "CLASS_COLUMN": "plantID" # Specify the class column for synthetic data generation
    }
}
...
SAMPLES_PER_CLASS_TARGET = 25 # Desired number of synthetic samples for EACH class (adjusted to 25)
...
# Global ECT min/max will be calculated dynamically within the main function
# to be specific to each dataset's ECT range.
...
def load_pca_model_data(pca_params_file: Path, pca_scores_labels_file: Path, class_column: str, exclude_classes: list):
    """
    ...
    with h5py.File(pca_scores_labels_file, 'r') as f:
        ...
        # Decode labels from bytes to string using the specified class_column
...
def generate_synthetic_pca_samples(pca_data: dict, samples_per_class_target: int, k_neighbors: int):
    ...
    Only generates samples to reach the 'samples_per_class_target' (topping up underrepresented classes).
    """
    print(f"\nStarting synthetic data generation (SMOTE-like) with target of {samples_per_class_target} samples per class...")
    ...
    total_generated_samples_overall = 0

    for class_name in all_classes:
        class_pca_samples = original_pca_scores[original_class_labels == class_name]
        current_samples_count = len(class_pca_samples)

        if current_samples_count >= samples_per_class_target:
            # print(f"Class '{class_name}' already has {current_samples_count} samples, meeting or exceeding target {samples_per_class_target}. No synthetic samples needed.")
            continue

        num_samples_to_generate = samples_per_class_target - current_samples_count
        print(f"Class '{class_name}': {current_samples_count} original samples. Generating {num_samples_to_generate} synthetic samples.")

        # Ensure enough distinct samples for NearestNeighbors for this class
        # k_neighbors must be less than or equal to (n_samples - 1)
        n_neighbors_for_class = min(current_samples_count - 1, k_neighbors)

        if n_neighbors_for_class < 1:
            print(f"Warning: Class '{class_name}' has insufficient samples ({current_samples_count}) for meaningful NearestNeighbors calculation (k={k_neighbors}). Skipping synthetic generation for this class.")
            continue

        nn = NearestNeighbors(n_neighbors=n_neighbors_for_class + 1).fit(class_pca_samples)

        generated_count_for_class = 0
        while generated_count_for_class < num_samples_to_generate:
            # Randomly select a sample from the current class
            idx_in_class_samples = np.random.randint(0, current_samples_count)
            sample = class_pca_samples[idx_in_class_samples]

            # Find its k nearest neighbors (including itself)
            distances, indices = nn.kneighbors(sample.reshape(1, -1))

            # Select a random neighbor (excluding the sample itself, which is at index 0 in `indices[0]`)
            available_neighbors_indices_in_class_pca = indices[0][1:]

            if len(available_neighbors_indices_in_class_pca) == 0:
                # This can happen if all points are identical, or k_neighbors is too high relative to unique points.
                # In such cases, we might re-evaluate `n_neighbors_for_class` or just try another base sample.
                continue
            ...
            total_generated_samples_overall += 1

    print(f"Finished generating a total of {total_generated_samples_overall} synthetic samples.")
...
def process_leaf_for_cnn_output(
    ...
    ect_min_val: float = None, # Renamed from global_ect_min
    ect_max_val: float = None  # Renamed from global_ect_max
):
    """
    ...
    try:
        ...
        # Pass dataset-specific ect_min_val and ect_max_val here for consistent scaling
        save_radial_ect_image(ect_result, shape_ect_path, cmap_name="gray", vmin=ect_min_val, vmax=ect_max_val)

        # --- Create Combined Visualization for Verification ---
        # Also pass dataset-specific ect_min_val and ect_max_val for consistent scaling in combined viz
        save_radial_ect_image(ect_result, temp_ect_inferno_path, cmap_name="inferno", vmin=ect_min_val, vmax=ect_max_val)
...
def calculate_ect_min_max_for_dataset(all_flattened_coords: np.ndarray, ect_calculator: ECT, apply_random_rotation: bool, dataset_name: str):
    """
    ...
    for i, flat_coords in enumerate(all_flattened_coords):
        # Print progress only when necessary, less frequently for this inner loop
        ...
        try:
            ...
            # print(f"  Warning: Skipping sample for ECT min/max calculation due to error: {e}")
...
def run_synthetic_data_pipeline_for_dataset(dataset_name: str, config: dict,
    ...
    Now accepts pre-calculated dataset-specific ect_min_val and ect_max_val.
    """
    ...
    synthetic_data_output_dir = config['SYNTHETIC_DATA_OUTPUT_BASE_DIR']
    ...
    # --- Combine Real and Synthetic Data for Processing ---
    # Prepare all coordinates and labels to be processed in one loop
    all_flattened_coords = np.vstack([
        pca_data['original_flattened_coords'],
        synthetic_flattened_coords
    ])
    all_class_labels = np.concatenate([
        pca_data['original_class_labels'],
        synthetic_y_labels
    ])
    all_is_real_flags = np.concatenate([
        np.full(len(pca_data['original_class_labels']), True),
        np.full(len(synthetic_y_labels), False)
    ])

    # Generate unique IDs for all samples (real and synthetic)
    all_sample_ids = [f"real_leaf_{i:05d}" for i in range(len(pca_data['original_class_labels']))] + \
                     [f"synthetic_leaf_{i:05d}" for i in range(len(synthetic_y_labels))]

    # Shuffle everything consistently to mix real and synthetic samples before processing
    # This helps in randomizing the progress prints and avoids processing all real then all synthetic.
    shuffled_indices = np.random.permutation(len(all_flattened_coords))
    all_flattened_coords = all_flattened_coords[shuffled_indices]
    all_class_labels = all_class_labels[shuffled_indices]
    all_is_real_flags = all_is_real_flags[shuffled_indices]
    all_sample_ids = np.array(all_sample_ids)[shuffled_indices].tolist()


    # --- Process All Samples (Real and Synthetic) ---
    total_samples_to_process = len(all_flattened_coords)
    print(f"\n--- Processing a total of {total_samples_to_process} (real + synthetic) samples for {dataset_name} ---")

    for i in range(total_samples_to_process):
        sample_id = all_sample_ids[i]
        class_label = all_class_labels[i]
        flat_coords = all_flattened_coords[i]
        is_real = all_is_real_flags[i]

        if (i + 1) % 100 == 0 or i == total_samples_to_process - 1:
            sample_type = "Real" if is_real else "Synthetic"
            print(f"Processing {sample_type} leaf {i+1}/{total_samples_to_process} ({sample_id}, Class: {class_label})")
        ...
        process_leaf_for_cnn_output(
            ...
            ect_min_val=ect_min_val,
            ect_max_val=ect_max_val
        )
...
if __name__ == "__main__":
    ...
    # --- Process each dataset independently (now only 'plant_predict') ---
    for dataset_name, config in DATASET_CONFIGS.items():
        ...
        # This will only generate if a class has fewer than SAMPLES_PER_CLASS_TARGET
        synthetic_X_pca_current_dataset, synthetic_y_labels_current_dataset = generate_synthetic_pca_samples(
        ...
        # This combined set is used ONLY for calculating the global ECT min/max for consistent scaling
        ...
        # The run_synthetic_data_pipeline_for_dataset function will now handle combining, shuffling,
        # and processing both real and synthetic data.
    ...
    print("\nAll synthetic data generation and processing pipelines completed.")