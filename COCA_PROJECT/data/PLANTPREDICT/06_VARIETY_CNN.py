...
K_FOLDS = 5
PATIENCE = 20 # Early stopping patience (for validation loss)
...
# Set this to the desired 3-letter variety prefix (e.g., "AMA", "CHA", "FIE")
# Set to None if you want to run on ALL varieties (original behavior)
VARIETY_PREFIX_FILTER = "AMA" # <<<--- SET YOUR VARIETY PREFIX HERE!
...
CNN_DATASET_CONFIGS = {
    ...
    "plant_predict": {
        ...
        "METADATA_FILE": Path("./05_synthetic_leaf_data_plant_predict/synthetic_metadata.csv") # NEW: Path to metadata
    }
}
...
def run_cnn_training_for_dataset(dataset_name: str, config: dict):
    ...
    metadata_file = config['METADATA_FILE'] # NEW: Get metadata file path
    dataset_nickname = config['DATASET_NICKNAME']
    
    # Adjust model identifier for filtered variety
    current_model_identifier_base = MODEL_IDENTIFIER_BASE
    if VARIETY_PREFIX_FILTER:
        current_model_identifier_base = f"{VARIETY_PREFIX_FILTER}_{MODEL_IDENTIFIER_BASE}"

    model_identifier = f"{dataset_nickname}_{current_model_identifier_base}"


    # Create dataset-specific output directories
    # Updated output directory name to reflect the filter
    base_output_dir = Path(f"./trained_models_{dataset_nickname}_{VARIETY_PREFIX_FILTER if VARIETY_PREFIX_FILTER else 'ALL'}/")
    ...
    try:
        ...
        X_images_full = final_data['X_images'] # (N, H, W, 2) numpy array
        y_labels_encoded_full = final_data['y_labels_encoded'] # (N,) numpy array
        is_real_flags_full = final_data['is_real_flags'] # (N,) boolean numpy array
        # No need to load sample_names or class_names from PKL, we'll get it from CSV
        class_names_original_order = final_data['class_names'] # Keep this for original mapping
        ...
        print(f"--- Loading metadata from {metadata_file} for filtering ---")
        metadata_df = pd.read_csv(metadata_file)
        
        # Ensure metadata_df 'synthetic_id' matches the order of X_images_full
        # The simplest way is to assume they are already aligned, or explicitly merge/reorder.
        # For now, let's assume the rows in metadata_df correspond to samples in X_images_full
        # based on the creation process of final_cnn_dataset.pkl.
        # If this causes issues, we'll need to use sample_names from final_data (if available)
        # to explicitly map. For now, we are trying to avoid changing 05_synthetic_leaf_data.py.
        
        # We need the 'class_label' from the metadata to determine the variety
        all_class_labels_from_metadata = metadata_df['class_label'].values
        # And the 'synthetic_id' from metadata to represent sample names
        all_sample_names_from_metadata = metadata_df['synthetic_id'].values

        # --- Apply Variety Prefix Filter ---
        if VARIETY_PREFIX_FILTER:
            print(f"\n--- Applying variety filter: Samples with class labels starting with '{VARIETY_PREFIX_FILTER}' ---")
            
            # Find indices where class_label starts with the filter prefix
            # Example: "AMA_1B" -> "AMA", "AMA_2M" -> "AMA"
            filter_mask = np.array([
                label.startswith(VARIETY_PREFIX_FILTER) for label in all_class_labels_from_metadata
            ])
            
            if not np.any(filter_mask):
                print(f"No samples found for variety prefix '{VARIETY_PREFIX_FILTER}'. Aborting training for this subset.")
                return

            X_images = X_images_full[filter_mask]
            y_labels_encoded_raw_filtered = y_labels_encoded_full[filter_mask]
            is_real_flags = is_real_flags_full[filter_mask]
            
            # Get the actual class names (e.g., 'AMA_1B', 'AMA_2M') for the filtered samples
            current_class_labels = all_class_labels_from_metadata[filter_mask]

            # Re-encode labels for the filtered subset
            # Create a new LabelEncoder specifically for the classes present after filtering
            label_encoder = LabelEncoder()
            label_encoder.fit(np.unique(current_class_labels)) # Fit on actual unique class strings
            
            # Get the new encoded labels
            y_labels_encoded = label_encoder.transform(current_class_labels)
            
            # Get the new class names in their sorted encoded order
            class_names = label_encoder.classes_.tolist()

            print(f"Filtered to {len(X_images)} samples for variety '{VARIETY_PREFIX_FILTER}'.")
            print(f"Number of classes after filtering: {len(class_names)}")

        else: # No filter applied, use all data
            X_images = X_images_full
            y_labels_encoded = y_labels_encoded_full
            is_real_flags = is_real_flags_full
            class_names = class_names_original_order # Use original class names
            # Recreate LabelEncoder from class_names_original_order for inverse_transform functionality
            label_encoder = LabelEncoder()
            label_encoder.fit(class_names_original_order)
            print("No variety filter applied. Using all samples.")


        # Consistent label for classification target, changed to reflect plantID
        target_column_used_for_data = 'plantID' 

        print(f"Loaded image data shape (after filter): {X_images.shape}")
        print(f"Number of classes (after filter): {len(class_names)}")
        print(f"Image size: {image_size_tuple}")
        print(f"Number of channels: {num_channels}")
        print(f"Number of real samples (after filter): {np.sum(is_real_flags)}")
        print(f"Number of synthetic samples (after filter): {np.sum(~is_real_flags)}")
        ...
        print(f"Error: Data file or Metadata file not found: {fnfe}.")
        print("Please ensure 'final_cnn_dataset.pkl' and 'synthetic_metadata.csv' exist at the specified paths.")
        return # Exit this dataset's processing
    except Exception as e:
        print(f"An unexpected error occurred while loading or filtering data for {dataset_name}: {e}")
        print(f"Details: {sys.exc_info()[0].__name__}: {e}")
        return # Exit this dataset's processing
    ...
    # Handle cases where there might not be enough real samples for K-folds after filtering
    if len(np.unique(y_original_for_skf)) < K_FOLDS or len(y_original_for_skf) < K_FOLDS:
        print(f"WARNING: Not enough real samples or classes for {K_FOLDS} folds after filtering.")
        print(f"Number of real samples: {len(y_original_for_skf)}, Number of unique real classes: {len(np.unique(y_original_for_skf))}")
        print("Will proceed with a single train/val split using all real data for validation and all synthetic+real for training.")
        
        # Adjust K_FOLDS to 1 for this scenario
        K_FOLDS_ADJUSTED = 1
        
        # Create a single "fold" where all real data is validation and all data is training
        train_original_real_indices = np.array([]) # No real data used for "training split" within real
        val_original_real_indices = np.arange(len(y_original_for_skf)) # All real data used for validation
        
        skf_splits_gen = [(train_original_real_indices, val_original_real_indices)] # Create a single tuple for iteration
        
    else:
        K_FOLDS_ADJUSTED = K_FOLDS
        skf_pytorch = StratifiedKFold(n_splits=K_FOLDS_ADJUSTED, shuffle=True, random_state=42)
        skf_splits_gen = skf_pytorch.split(X_original_images_for_skf.cpu().numpy(), y_original_for_skf.cpu().numpy())


    all_predictions_logits = []
    saved_model_paths_per_fold = [None] * K_FOLDS_ADJUSTED

    # --- Calculate class weights for imbalanced dataset ---
    # Class weights should be based on the *training* labels for the current filtered subset
    # Make sure we're using the correct y_labels_encoded which has been filtered and re-encoded
    all_training_labels_for_weights = y_encoded_tensor.cpu().numpy()
    
    # Ensure there are unique classes to compute weights for
    if len(np.unique(all_training_labels_for_weights)) > 1:
        ...
        # If only one class exists, weights are not needed (or set to 1)
        class_weights_tensor = torch.tensor([1.0], dtype=torch.float).to(device)
    
    print(f"\nCalculated class weights: {class_weights_tensor.cpu().numpy()}")

    for fold_idx, (train_original_real_indices, val_original_real_indices) in enumerate(skf_splits_gen):
        print(f"\n--- Fold {fold_idx + 1}/{K_FOLDS_ADJUSTED} ---")
        ...
        # Ensure global_real_train_indices is correctly derived based on the current overall filtered dataset
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            with torch.no_grad():
                ...
                if len(val_loader.dataset) == 0:
                    avg_val_loss = float('inf') # Set to inf if no validation samples
                    val_accuracy = 0.0
                else:
    ...
    # Ensure class_names passed to classification_report matches the *filtered* class_names
    ...
    if saved_model_paths_per_fold[0] is None or not Path(saved_model_paths_per_fold[0]).exists():
        ...
        # real_original_indices_global refers to indices within the *filtered* dataset
