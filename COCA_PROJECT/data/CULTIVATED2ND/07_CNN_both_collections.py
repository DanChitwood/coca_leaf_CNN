...
MODEL_IDENTIFIER_BASE = 'ECT_Mask_2Channel_CNN_Ensemble_CombinedCollections' # Base identifier, will be prefixed

# --- Dataset-Specific Configuration for Loading ---
# This dictionary holds configurations for each dataset you want to train on.
# These paths point to the outputs of 04_synthetic_data_generation.py
...
def run_cnn_training_for_combined_datasets(configs: dict):
    ...
    print(f"Created output directories for combined datasets under {base_output_dir}")
    ...
    for i, original_encoded_labels in enumerate(all_y_labels_encoded):
        # We need to inverse transform original encoded labels to string, then encode with the new global encoder
        # This assumes the original label_encoder for each file used was consistent with its own class_names
        # A safer approach might be to load raw string labels from metadata in 04, but this should work
        original_class_names_for_dataset = sorted(list(all_class_names_sets[i])) # Reconstruct original order
        temp_le = LabelEncoder()
        temp_le.fit(original_class_names_for_dataset)
        
        # Convert original encoded labels to string names using original encoder
        original_string_labels = temp_le.inverse_transform(original_encoded_labels)
        
        # Then encode string names with the new global encoder
    ...
    for fold_idx, (train_original_real_indices, val_original_real_indices) in enumerate(skf_pytorch.split(X_original_images_for_skf.cpu().numpy(), y_original_for_skf.cpu().numpy())):
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            total_samples = 0
            current_overall_accuracy_on_real_samples = 0.0 # Placeholder for printing
    ...
    if saved_model_paths_per_fold[0] is None or not Path(saved_model_paths_per_fold[0]).exists():
        ...
        plt.suptitle(f'Average Grad-CAM Visualizations ({model_identifier}, Target: Leaf_Class_Combined)', fontsize=16, y=1.02)
