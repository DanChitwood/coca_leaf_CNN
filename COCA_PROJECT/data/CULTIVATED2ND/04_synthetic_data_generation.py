...
DATASET_CONFIGS = {
    "cultivated1st": {
        "SAVED_MODEL_DIR": Path("./03_morphometrics_output_cultivated1st_combined/"),
        "PCA_PARAMS_FILE_NAME": "leaf_pca_model_parameters_cultivated1st.h5",
        "PCA_SCORES_LABELS_FILE_NAME": "original_pca_scores_and_class_labels_cultivated1st.h5",
        "EXCLUDE_CLASSES": [], # No classes to exclude for cultivated1st
        "SYNTHETIC_DATA_OUTPUT_BASE_DIR": Path("./05_synthetic_leaf_data_cultivated1st/"),
        "DATASET_FULL_NAME": "Cultivated 1st Collection"
    },
    "cultivated2nd": {
        "SAVED_MODEL_DIR": Path("./03_morphometrics_output_cultivated2nd_combined/"),
        "PCA_PARAMS_FILE_NAME": "leaf_pca_model_parameters_cultivated2nd.h5",
        "PCA_SCORES_LABELS_FILE_NAME": "original_pca_scores_and_class_labels_cultivated2nd.h5",
        "EXCLUDE_CLASSES": ["DES", "POM", "BON"], # Exclude these classes for cultivated2nd
        "SYNTHETIC_DATA_OUTPUT_BASE_DIR": Path("./05_synthetic_leaf_data_cultivated2nd/"),
...
def load_pca_model_data(pca_params_file: Path, pca_scores_labels_file: Path, exclude_classes: list):
    """
...
def calculate_ect_min_max_for_dataset(all_flattened_coords: np.ndarray, ect_calculator: ECT, apply_random_rotation: bool, dataset_name: str):
    ...
    for i, flat_coords in enumerate(all_flattened_coords):
        if (i + 1) % 500 == 0 or i == num_samples - 1: # Print less frequently for this inner loop
...
def run_synthetic_data_pipeline_for_dataset(dataset_name: str, config: dict,
                                            ect_min_val: float, ect_max_val: float, # Renamed parameters
    ...
    pca_data = load_pca_model_data(pca_params_file_path, pca_scores_labels_file_path, config['EXCLUDE_CLASSES'])
    ...
    # --- 7. Process Original Real Samples (now with dataset-specific ECT min/max) ---
    ...
    # --- 8. Process Synthetic PCA Samples (now with dataset-specific ECT min/max) ---
...
if __name__ == "__main__":
    ...
    for dataset_name, config in DATASET_CONFIGS.items():
        ...
        pca_data_current_dataset = load_pca_model_data(
            pca_params_file_path, pca_scores_labels_file_path, config['EXCLUDE_CLASSES']
        )
