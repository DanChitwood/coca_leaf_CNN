...
# Updated MODEL_IDENTIFIER_BASE for variety-by-collection classification
MODEL_IDENTIFIER_BASE = 'ECT_Mask_2Channel_CNN_Ensemble_VarietyByCollection'
...
def run_cnn_training_for_variety_by_collection(configs: dict):
    """
    Runs the full CNN training and evaluation pipeline for variety-by-collection datasets.
    """
    print(f"\n{'='*10} Starting CNN Training for Variety-by-Collection Datasets {'='*10}")

    # --- Setup Dynamic Paths and Identifiers for Variety-by-Collection Dataset ---
    dataset_nickname = "VarietyByCollection" # Identifier for combined output
    ...
    print(f"Created output directories for Variety-by-Collection datasets under {base_output_dir}")

    # --- Data Loading and Combination with New Labels ---
    all_X_images = []
    # Store raw string labels for the new combined labels
    all_y_string_labels_with_collection = [] 
    all_is_real_flags = []
    ...
    print(f"\n--- Loading and combining data from multiple datasets with collection-specific labels ---")
    for ds_name, ds_config in configs.items():
        data_file_path = ds_config['FINAL_PREPARED_DATA_FILE']
        collection_suffix = '_1st' if ds_name == "cultivated1st" else '_2nd'
        print(f"Loading {ds_name} data from {data_file_path} and adding '{collection_suffix}' suffix...")
        try:
            ...
            # Reconstruct original string labels from encoded labels for this dataset
            # This requires having the original label_encoder or class_names for this specific .pkl file
            # Assuming 'class_names' key exists in your .pkl files for this purpose
            original_class_names_for_dataset = sorted(list(data['class_names']))
            temp_le = LabelEncoder()
            temp_le.fit(original_class_names_for_dataset)
            original_string_labels = temp_le.inverse_transform(data['y_labels_encoded'])

            # Create new labels by combining original label with collection suffix
            new_y_labels_with_suffix = [f"{label}{collection_suffix}" for label in original_string_labels]
            all_y_string_labels_with_collection.extend(new_y_labels_with_suffix)
    ...
    # Create a unified LabelEncoder for the NEW variety-by-collection labels
    label_encoder = LabelEncoder()
    # Fit on all unique string labels (e.g., 'amazona_1st', 'amazona_2nd', etc.)
    y_labels_encoded = label_encoder.fit_transform(all_y_string_labels_with_collection)
    
    class_names = list(label_encoder.classes_) # Get the new ordered class names
    
    image_size_tuple = first_image_size
    num_channels = first_num_channels

    print(f"\n--- Variety-by-Collection Dataset Statistics ---")
    print(f"Total loaded image data shape: {X_images.shape}")
    print(f"Number of classes (variety_by_collection): {len(class_names)}")
    print(f"Class names (variety_by_collection): {class_names}")
    ...
    print(f"Data will be processed for classification of: 'Leaf_Variety_by_Collection'")
    ...
    print(f"\n--- Performing PyTorch CNN with {K_FOLDS}-Fold Stratified Cross-Validation (2-Channel Image Data) on Variety-by-Collection Dataset ---")
    ...
    # Stratified K-Fold splitting should be based on the NEW variety-by-collection labels
    ...
    # Class weights now calculated based on the NEW variety-by-collection labels
    ...
    print(f"\nCalculated class weights (based on combined real and synthetic data, variety-by-collection): {class_weights_tensor.cpu().numpy()}")
    ...
    print(f"\n--- Overall Accuracy ({model_identifier} Ensemble, Evaluated on ALL REAL samples - Leaf_Variety_by_Collection): {overall_accuracy_real_pt:.4f} ---")

    print(f"\n--- Classification Report ({model_identifier} Ensemble, Evaluated on ALL REAL samples - Leaf_Variety_by_Collection) ---")
    ...
    plt.figure(figsize=(20, 18)) # Adjusted size for potentially more classes
    sns.heatmap(cm_real_pt, annot=True, fmt='d', cmap='Blues', cbar=False,
                xticklabels=class_names, yticklabels=class_names)
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.title(f'Confusion Matrix ({model_identifier} Ensemble, Evaluated on ALL REAL samples - Leaf_Variety_by_Collection)')
    ...
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.title(f'Normalized Confusion Matrix ({model_identifier} Ensemble, Evaluated on ALL REAL samples - Leaf_Variety_by_Collection)')
    plt.xticks(rotation=90)
    plt.yticks(rotation=0)
    plt.tight_layout()
    ...
    results_storage['Variety_by_Collection_Dataset'] = {
        'class_counts': {},
        'model_metrics': {}
    }
    ...
    for encoded_label, count in class_counts_series.items():
        ...
        results_storage['Variety_by_Collection_Dataset']['class_counts'][class_name_str] = count
    print(f"Class counts populated for 'Variety_by_Collection_Dataset'.")

    results_storage['Variety_by_Collection_Dataset']['model_metrics'][model_identifier] = {
    ...
    print(f"Metrics for '{model_identifier}' stored in results_storage for 'Variety_by_Collection_Dataset'.")
    ...
    if saved_model_paths_per_fold[0] is None or not Path(saved_model_paths_per_fold[0]).exists():
        ...
        plt.suptitle(f'Average Grad-CAM Visualizations ({model_identifier}, Target: Leaf_Variety_by_Collection)', fontsize=16, y=1.02)
        plt.tight_layout(rect=[0, 0.03, 1, 0.98])
        plt.savefig(os.path.join(base_output_dir, f"{model_identifier}_AverageGradCAM.png"), dpi=300)
        plt.show()

    print(f"\n{'='*10} Finished CNN Training for Variety-by-Collection Datasets {'='*10}")


if __name__ == "__main__":
    run_cnn_training_for_variety_by_collection(CNN_DATASET_LOAD_CONFIGS)

    print("\nAll CNN training and evaluation pipelines for Variety-by-Collection datasets completed.")
