...
FINAL_PREPARED_DATA_FILE = Path("./05_synthetic_leaf_data/final_cnn_dataset_plantID_level.pkl")
...
try:
    ...
    # Renamed variables to match the new structure from the previous script
    X_plantid_collections = final_data['X_plantid_collections'] # (Num_PlantIDs, H, W, 2*n) numpy array
    y_plantid_labels_encoded = final_data['y_plantid_labels_encoded'] # (Num_PlantIDs,) numpy array
    plantid_is_fully_real_collection = final_data['plantid_is_fully_real_collection'] # (Num_PlantIDs,) boolean numpy array
    plantid_class_names = final_data['plantid_class_names'] # List of string class names
    image_size_per_leaf = final_data['image_size_per_leaf'] # (H, W) tuple, e.g., (256, 256)
    num_leaves_per_plantid = final_data['num_leaves_per_plantid'] # int, should be 10
    total_channels_per_plantid = final_data['total_channels_per_plantid'] # int, should be 2*10=20
    ...
    # Assuming classification is by 'PlantID' now
    ...
    print(f"Number of fully real PlantID collections: {np.sum(plantid_is_fully_real_collection)}")
    print(f"Number of mixed/synthetic PlantID collections: {np.sum(~plantid_is_fully_real_collection)}")
    ...
    print("Please ensure the data generation script (04_main_synthetic_generation.py) has been run successfully.")
    sys.exit(1)
...
# This flag is for informational purposes for now, not directly used in dataset __getitem__
# as train/val split is handled by StratifiedKFold on the whole dataset.
is_fully_real_collection_tensor = torch.from_numpy(plantid_is_fully_real_collection).bool()
