...
SAVED_MODEL_DIR = Path("./outputs/") # Path to the directory where PCA model and scores were saved
...
SYNTHETIC_DATA_OUTPUT_DIR = Path("./02_synthetic_leaf_data/") # <<< CHANGED TO 02_synthetic_leaf_data >>>
...
def save_radial_ect_image(ect_result, save_path: Path, cmap_name: str = "gray", vmin: float = None, vmax: float = None):
    ...
    fig, ax = plt.subplots(subplot_kw=dict(projection="polar"),
                           ...
                           facecolor='white') # MODIFIED: Set facecolor to white
    ...
    # Save the figure
    # MODIFIED: Pass fig.get_facecolor() to savefig for consistent background
...
def create_combined_viz_from_images(ect_image_path: Path, overlay_coords: np.ndarray,
    ...
    Leaf outline component to black.
    """
    try:
...
def process_leaf_for_cnn_output(
    ...
    # Temporary path for ECT image used in combined viz (will be deleted)
    ...
    try:
        ...
        # Radial ECT images by themselves are NORMAL grayscale
...
def main_synthetic_generation(clear_existing_data: bool = True):
    ...
    # This ensures that ECTs of both real and synthetic data are scaled consistently.
