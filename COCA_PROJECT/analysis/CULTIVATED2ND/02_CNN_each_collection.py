...
# Parents[0] is 'CULTIVATED2ND', parents[1] is 'analysis', parents[2] is 'COCA_PROJECT'
PROJECT_ROOT = Path(__file__).resolve().parents[2]
ANALYSIS_DIR = PROJECT_ROOT / "analysis"
CURRENT_SCRIPT_DIR = Path(__file__).parent # This will be COCA_PROJECT/analysis/CULTIVATED2ND
...
# ALL data files and ALL model outputs will be relative to CURRENT_SCRIPT_DIR (i.e., COCA_PROJECT/analysis/CULTIVATED2ND/)
CNN_DATASET_CONFIGS = {
    "cultivated1st": {
        "DATASET_NICKNAME": "Cultivated1st", # A short name for file paths and identifiers
        # Input path for processed data from 01_synthetic_leaf_data.py
        # Data files are now correctly located within CURRENT_SCRIPT_DIR
        "FINAL_PREPARED_DATA_FILE": CURRENT_SCRIPT_DIR / "01_synthetic_leaf_data_cultivated1st" / "final_cnn_dataset.pkl",
        # Output directory for trained models and metrics
        # Output models should also be in CURRENT_SCRIPT_DIR
        "MODEL_OUTPUT_BASE_DIR": CURRENT_SCRIPT_DIR / "02_trained_models_Cultivated1st"
    },
    "cultivated2nd": {
        ...
        # Output directory for trained models and metrics
        # Output models should also be in CURRENT_SCRIPT_DIR
        "MODEL_OUTPUT_BASE_DIR": CURRENT_SCRIPT_DIR / "02_trained_models_Cultivated2nd"
    }
}
...
def run_cnn_training_for_dataset(dataset_name: str, config: dict):
    ...
    Args:
        dataset_name (str): The internal name of the dataset (e.g., "cultivated1st").
        config (dict): Configuration dictionary for the current dataset.
    """
    ...
    base_output_dir = config['MODEL_OUTPUT_BASE_DIR']
    ...
    try:
        ...
        print("Please ensure the data generation script (e.g., 01_synthetic_leaf_data.py) has been run successfully.")
    ...
    print(f"\n--- Performing PyTorch CNN with {K_FOLDS}-Fold Stratified Cross-Validation ---")
    ...
    # Initialize StratifiedKFold with the global random seed
    ...
    print(f"\n--- Overall Accuracy ({model_identifier} Ensemble, Evaluated on REAL samples ONLY): {overall_accuracy_real_pt:.4f} ---")

    print(f"\n--- Classification Report ({model_identifier} Ensemble, Evaluated on REAL samples ONLY) ---")
    ...
    plt.title(f'Confusion Matrix ({model_identifier} Ensemble, Evaluated on REAL samples ONLY)')
    plt.xticks(rotation=90)
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig(base_output_dir / f"{model_identifier}_ConfusionMatrix.png", dpi=300)
    plt.show()
    ...
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.title(f'Normalized Confusion Matrix ({model_identifier} Ensemble, Evaluated on REAL samples ONLY)')
    plt.xticks(rotation=90)
    plt.yticks(rotation=0)
    plt.tight_layout()
    plt.savefig(base_output_dir / f"{model_identifier}_NormalizedConfusionMatrix.png", dpi=300)
    plt.show()
    ...
    # Use the first saved model for Grad-CAM visualization
    model_to_visualize_path = saved_model_paths_per_fold[0]

    if model_to_visualize_path is None or not Path(model_to_visualize_path).exists():
        ...
        target_layer = cam_model.features[8]
        ...
        for class_idx in range(len(class_names)):
            ...
            # Randomly sample images for CAM calculation to get a representative average
            # Use np.random.default_rng() for modern NumPy random state management
            rng = np.random.default_rng(GLOBAL_RANDOM_SEED + class_idx) # Use a different seed per class for varied samples
        ...
        plt.savefig(base_output_dir / f"{model_identifier}_AverageGradCAM.png", dpi=300)
        plt.show()
...
if __name__ == "__main__":
    ...
    print(json.dumps(results_storage, indent=4)) # Pretty print the final results storage