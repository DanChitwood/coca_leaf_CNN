...
MODEL_OUTPUT_BASE_DIR = CURRENT_SCRIPT_DIR / "04_trained_models_collections"
...
class LeafDataset(Dataset):
    ...
    Adjusted for binary classification where labels are expected as float and unsqueezed.
    """
...
class LeafCNN(tnn.Module):
    ...
    Designed to output a single logit for binary classification.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: The output logits for each class (for binary, this will be a [batch_size, 1] tensor).
        """
        x = self.features(x)
        x = self.classifier(x)
        return x
...
class GradCAM:
    ...
    def __call__(self, input_tensor: torch.Tensor, target_class: int = None) -> np.ndarray:
        """
        Computes the Grad-CAM heatmap for a given input tensor in a binary classification context.

        Args:
            input_tensor (torch.Tensor): The input image tensor (1, C, H, W).
            target_class (int, optional): The target class index (0 or 1).
                                          If None, the predicted class (based on logit threshold 0) is used.

        Returns:
            np.ndarray: The computed Grad-CAM heatmap, resized to input image dimensions.
        """
        self.model.zero_grad()
        output = self.model(input_tensor) # output is a logit (single value)
        
        if target_class is None: # Use the predicted class if not specified
            predicted_class_logit = output.item()
            target_class = 1 if predicted_class_logit >= 0 else 0 # Threshold 0 for logit to get binary class
...
def run_cnn_training_for_collection_binary(configs: dict):
    ...
    print(f"Created output directories for Collection Binary Classification under {MODEL_OUTPUT_BASE_DIR}")
    ...
    for ds_name, ds_config in configs.items():
        ...
        try:
            ...
            print(f"Error: Data file not found at {data_file_path}. Skipping this dataset. Exiting.")
    ...
    # Define the two class names for clarity in reports
    class_names = ['Collection_1st', 'Collection_2nd']
    num_classes = 1 # Output dimension of the model is 1 for binary classification (single logit)
    ...
    y_encoded_tensor = torch.from_numpy(y_labels_encoded).long() # Labels are 0/1 integers
    ...
    # --- Calculate pos_weight for BCEWithLogitsLoss (on ALL training labels: real + synthetic) ---
    ...
    if saved_model_paths_per_fold[0] is None or not Path(saved_model_paths_per_fold[0]).exists():
        ...
        for class_idx in range(len(class_names)): # Will iterate for 0 and 1
            ...
            for sample_idx in samples_for_cam:
                ...
                # We specify the true class for the sample.
        ...
        # Remove any unused subplots (if num_plots_total is less than num_rows_grid * num_cols_grid)
