...
# Base directory for data and figures, relative to the location of this script (COCA_PROJECT/notebooks).
BASE_PROJECT_DIR = Path("../")

# Paths to the JSON files for each dataset
# Updated paths to reflect the new structure relative to COCA_PROJECT/notebooks/
JSON_PATHS = {
    "1st_Dataset": BASE_PROJECT_DIR / "data" / "CULTIVATED2ND" / "trained_models_Cultivated1st" / "metrics" / "Cultivated1st_ECT_Mask_2Channel_CNN_Ensemble_Improved_classification_report.json",
    "2nd_Dataset": BASE_PROJECT_DIR / "data" / "CULTIVATED2ND" / "trained_models_Cultivated2nd" / "metrics" / "Cultivated2nd_ECT_Mask_2Channel_CNN_Ensemble_Improved_classification_report.json",
}
...
OUTPUT_TABLE_FILENAME = "Cultigen_Performance_Summary_Table.csv" # New filename to prevent overwriting
...
for class_name in SHARED_CLASSES:
    ...
    for dataset_prefix, ds_name in [("1st", "1st_Dataset"), ("2nd", "2nd_Dataset")]:
