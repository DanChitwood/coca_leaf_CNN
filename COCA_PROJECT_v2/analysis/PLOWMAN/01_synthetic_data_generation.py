...
# --- Project Root and Input Data Configuration (from previous script's output) ---
# Define the root project directory relative to the script's location
PROJECT_ROOT = Path(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../')))

# Input files from 02_plowman_morphometric_analysis.py
# Updated to point to the correct output folder from the previous script
PCA_PARAMS_FILE = PROJECT_ROOT / "analysis" / "PLOWMAN" / "outputs" / "plowman_pca_model_parameters.h5"
PCA_SCORES_LABELS_FILE = PROJECT_ROOT / "analysis" / "PLOWMAN" / "outputs" / "plowman_original_pca_scores_and_class_labels.h5"
...
# This is the new output base directory, directly under PLOWMAN
SYNTHETIC_DATA_OUTPUT_DIR = PROJECT_ROOT / "analysis" / "PLOWMAN" / "02_synthetic_leaf_data"
...
def process_leaf_for_cnn_output(
    ...
    # Temporary ECT image path for combined visualization
    ...
    try:
        ...
        current_metadata["file_shape_ect"] = str(shape_ect_path.relative_to(SYNTHETIC_DATA_OUTPUT_DIR)) # Corrected to shape_ect_path
        ...
        # Corrected: Use temp_ect_combined_viz_path instead of undefined ect_path
...
def main_synthetic_generation(clear_existing_data: bool = True):
    ...
    print(f"Created output directories under: {SYNTHETIC_DATA_OUTPUT_DIR}")
