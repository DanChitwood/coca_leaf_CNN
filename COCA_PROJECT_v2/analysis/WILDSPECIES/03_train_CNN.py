...
FINAL_PREPARED_DATA_FILE = Path("./02_synthetic_leaf_data/final_cnn_dataset.pkl") # CORRECTED PATH
...
def set_seed(seed):
    ...
    if torch.backends.mps.is_available(): # For MPS (Apple Silicon)
        # MPS doesn't have a direct manual_seed equivalent like CUDA
        # Operations are generally deterministic on MPS when not using specific non-deterministic ops
        pass 
...
try:
    ...
    print("Please ensure the data generation script (05_synthetic_leaf_data.py) has been run successfully.")
    sys.exit(1)
...
skf_pytorch = StratifiedKFold(n_splits=K_FOLDS, shuffle=True, random_state=RANDOM_SEED)
...
if len(saved_model_paths_per_fold) <= 0 or saved_model_paths_per_fold[0] is None or not os.path.exists(saved_model_paths_per_fold[0]):
    ...
    for class_idx in range(len(class_names)):
        ...
        # Ensure that if there are fewer than NUM_SAMPLES_PER_CLASS_FOR_CAM_PLOT samples, we use all of them
    ...
    for i in range(len(class_names)):
        ...
        if avg_heatmap is not None:
            if real_indices_by_class[i]:
                # Use a specific real sample for background to ensure consistency if multiple samples
                # are available, or pick the first available if not enough for random choice.
                # For consistency, just pick the first sample from the real_indices_by_class list.
