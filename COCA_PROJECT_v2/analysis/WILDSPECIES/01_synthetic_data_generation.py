...
import sys
import shutil
import cv2 # For affine transformation matrix calculation
import os
import h5py # For loading PCA model parameters and scores
import pickle # For saving the final CNN dataset
from sklearn.neighbors import NearestNeighbors # For SMOTE-like augmentation
...
# Define the directory where morphometrics outputs (PCA model/scores) are located
MORPHOMETRICS_OUTPUT_DIR = WILDSPECIES_ANALYSIS_DIR / "outputs"

# Define the output directory for synthetic data and processed images
SYNTHETIC_DATA_OUTPUT_DIR = WILDSPECIES_ANALYSIS_DIR / "02_synthetic_leaf_data"
...
PCA_PARAMS_FILE = MORPHOMETRICS_OUTPUT_DIR / "leaf_pca_model_parameters.h5"
PCA_SCORES_LABELS_FILE = MORPHOMETRICS_OUTPUT_DIR / "original_pca_scores_and_class_labels.h5"
...
# Grayscale values for output mask file (for CNN input)
...
OUTLINE_COLOR = (0, 0, 0) # Black outline for combined_viz images
ECT_CMAP_COMBINED_VIZ = "gray_r" # Reverse grayscale for ECT part in combined_viz
COMBINED_VIZ_BACKGROUND_COLOR = (255, 255, 255) # White background for combined_viz images
...
# For Procrustes-aligned data, this should typically be False.
...
SYNTHETIC_COMBINED_VIZ_DIR = SYNTHETIC_DATA_OUTPUT_DIR / "combined_viz" # Contains combined visualizations

SYNTHETIC_METADATA_FILE = SYNTHETIC_DATA_OUTPUT_DIR / "synthetic_metadata.csv"

# --- Final Consolidated Data Output for CNN Training ---
FINAL_PREPARED_DATA_FILE = SYNTHETIC_DATA_OUTPUT_DIR / "final_cnn_dataset.pkl"

# Global ECT min/max will be calculated dynamically for consistent image scaling
GLOBAL_ECT_MIN = None
GLOBAL_ECT_MAX = None

print(f"WILDSPECIES analysis directory: {WILDSPECIES_ANALYSIS_DIR}")
print(f"PCA model inputs from: {MORPHOMETRICS_OUTPUT_DIR}")
print(f"Synthetic data and image outputs to: {SYNTHETIC_DATA_OUTPUT_DIR}")
...
def save_radial_ect_image(ect_result, save_path: Path, cmap_name: str, vmin: float = None, vmax: float = None):
    """
    ...
    # Set background color for the figure and axes to white (using string name)
    ...
    plt.savefig(save_path, bbox_inches='tight', pad_inches=0, dpi=100, facecolor='white') # Explicitly set facecolor to 'white'
...
def create_combined_viz_from_images(ect_image_path: Path, overlay_coords: np.ndarray,
    ...
    The ECT image provided is expected to already have the desired background color.
    """
    try:
        ...
        # Since ect_img is already expected to have a white background and no transparency
        # when it was saved, alpha_composite will correctly blend transparent overlay.
    ...
    with h5py.File(pca_scores_labels_file, 'r') as f:
        ...
        if 'original_flattened_coords' in f:
            ...
            print("Warning: 'original_flattened_coords' not found in PCA_SCORES_LABELS_FILE. "
                  ...
                  "Ensure your previous morphometrics script saves this data.")
...
def process_leaf_for_cnn_output(
    ...
    # Temporary file for the ECT image used in combined visualization
    ...
    try:
        ...
        save_radial_ect_image(ect_result, shape_ect_path, cmap_name="gray_r", vmin=global_ect_min, vmax=global_ect_max)

        # --- Create Combined Visualization for Verification ---
        # Save the ECT image with reverse grayscale and white background for combined viz
        save_radial_ect_image(ect_result, temp_ect_combined_viz_path, cmap_name=ECT_CMAP_COMBINED_VIZ, vmin=global_ect_min, vmax=global_ect_max)
        
        create_combined_viz_from_images(
            temp_ect_combined_viz_path, transformed_contour_for_mask, combined_viz_path,
            overlay_color=OUTLINE_COLOR, overlay_alpha=1.0, # Black outline, full opacity
        ...
        # Clean up temporary ECT image used for combined visualization
...
def calculate_global_ect_min_max(all_flattened_coords: np.ndarray, ect_calculator: ECT, apply_random_rotation: bool):
    ...
    for i, flat_coords in enumerate(all_flattened_coords):
        ...
        try:
            ...
            # print(f"  Error calculating ECT for sample {i} for global min/max: {e}") # Uncomment for debugging
...
def main_synthetic_generation(clear_existing_data: bool = True):
    ...
    # --- 6. Calculate Global ECT Min/Max ---
...
if __name__ == "__main__":
    # Set clear_existing_data to True to remove previous outputs in SYNTHETIC_DATA_OUTPUT_DIR
