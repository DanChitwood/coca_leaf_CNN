...
SAVED_MODEL_DIR = Path("./03_morphometrics_output/") # Path to the directory where PCA model and scores were saved
...
def save_radial_ect_image(ect_result, save_path: Path, cmap_name: str = "gray"):
    ...
    # Use pcolormesh to plot the ECT data
    im = ax.pcolormesh(THETA, R, ect_result.T, cmap=cmap_name) # ect_result.T for correct orientation
...
def process_leaf_for_cnn_output(
    ...
    apply_random_rotation: bool = False # NEW PARAMETER
):
    """
    ...
    Includes an option to apply random rotation.
    """
    current_metadata = {
    ...
    try:
        ...
        if apply_random_rotation:
            ...
            print(f"  Applied random rotation of {random_angle_deg:.2f} degrees to {sample_id}.")
        else:
            processed_contour_coords = raw_contour_coords.copy() # Use original (aligned) coords
            print(f"  Skipping random rotation for {sample_id} (Procrustes alignment maintained).")
        # --- END Random Rotation ---
        ...
        save_radial_ect_image(ect_result, shape_ect_path, cmap_name="gray")

        # --- Create Combined Visualization for Verification ---
        save_radial_ect_image(ect_result, temp_ect_inferno_path, cmap_name="inferno")
...
def main_synthetic_generation(clear_existing_data: bool = True):
    ...
    # --- 4. Process Original Real Samples ---
    ...
    for i in range(num_real_samples):
        ...
        process_leaf_for_cnn_output(
            ...
            metadata_records,
            is_real_sample=True,
            apply_random_rotation=APPLY_RANDOM_ROTATION # Pass the configuration parameter
        )

    # --- 5. Generate and Process Synthetic PCA Samples ---
    ...
    for i in range(total_synthetic_samples):
        ...
        if len(flat_coords) != FLATTENED_COORD_DIM:
            print(f"Error: Reconstructed flattened coordinates for {sample_id} have unexpected dimension {len(flat_coords)}. Expected {FLATTENED_COORD_DIM}. Skipping.")
            metadata_records.append({
                ...
                "num_contour_coords": 0,
                "file_shape_mask": "", "file_shape_ect": "",
                "file_combined_viz": "",
                "is_real": False
            })
